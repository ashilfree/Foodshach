...
class CouponController extends AbstractController
{
    ...
    /**
     * @Route("/{locale}/coupon", name="coupon.store", defaults={"locale"="en"})
     * @param $locale
     ...
     * @throws \Exception
     */
    public function store($locale, CouponRepository $couponRepository, Request $request): Response
    {
        $form = $this->createForm(CouponType::class);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            $coupon = $couponRepository->findByCode($form->get('code')->getData());
            if(!$coupon){
